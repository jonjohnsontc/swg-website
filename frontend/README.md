...
* [`src/cljs/swg_website/`](src/cljs/swg_website/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/cljs/swg_website/core.cljs): contains the SPA entry point, `init`
...
7. Clone this repo and open a terminal in the `swg-website` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/swg_website/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns swg-website.example
  (:require [swg-website.config :as config])
